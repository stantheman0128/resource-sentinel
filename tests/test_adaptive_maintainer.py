...
class AdaptiveMaintainerTests(unittest.TestCase):
    ...
    def add_local(self, name="local-a", *, canonical="host-fixture", scope="SHARED_POOL"):
        caps = dict(local=True, adapter_ready=True, admission_policy="resource-v2",
    ...
    @staticmethod
    def task(name, ram=30, *, worker="local-a", **kwargs):
        return Task(name, ram_gib=ram, cpu_units=.25, io_slots=0,
                    execution_preference="LOCAL_REQUIRED", allowed_worker_ids=(worker,), **kwargs)
    ...
    def test_host_binding_does_not_depend_on_worker_alias(self):
        with patch("sentinel.maintainer.socket.gethostname", return_value="Fixture-HOST"):
